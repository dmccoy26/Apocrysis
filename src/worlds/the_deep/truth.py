...
docs/WORLD_3_THE_DEEP.md §5B.4.

Content only - same shape as worlds/silence/truth.py and
worlds/the_wake/truth.py. Bound to escape mechanisms by discovery.py;
solving a mechanism surfaces the fact it carries. Integrity (acyclic,
...
galleries behind them and cut the line from the deep side so no one
could order them back. They have been the containment ever since.
Restoring the site to reach the survivors rebuilds the exact road the
seam travels up.

Kill-test 0 note: several Band IV-V facts are spec'd to land through
`WorldContact` (kill-test B) and a `campaign_state` check (kill-test A).
Until those exist they are carried by ordinary mechanism routes
(discovery.py) and, for the deepest tail, absorbed by the finale's
`also_establishes`.
"""
...
MILESTONE_IDS = frozenset({
    ...
    'CHANGED_HAVE_STRUCTURE',      # M2  the Changed hold ground - they are stationed
    'COMPANY_KNEW',               # M3  the company established the harm and kept mining
...
WORLD_FACTS = (
    ...
    WorldFact('COMPANY_CORRELATION', 'order', 3, False,
              "The occupational records don't just log the symptoms - they model them. Exposure in, severity out. The company had the curve.",
              lead="did the company connect it",
              needs=('ANOMALY_REPORTS',)),
    WorldFact('QUOTAS_CONTINUED', 'order', 3, False,
              "Rotations, exposure caps, masks, monitoring - a managed-harm programme, running side by side with extraction targets that never once came down.",
              lead="did production ever slow",
              needs=('COMPANY_CORRELATION',)),
    ...
    WorldFact('WORKERS_CHOSE_ISOLATION', 'facility', 4, False,
              "One of them told me straight: the deep crews put the pattern together, downed tools, sealed the last galleries behind them and pulled the line so nobody could order them back.",
              lead="why the deep crews sealed themselves in",
              needs=('COMMS_CUT_FROM_BELOW',)),
    WorldFact('CASE_TIMING', 'seam', 4, False,
              "The first cases outside this site track the shipment manifests and the crews rotating out. It didn't spread on the wind. It rode the ore and the people.",
              lead="how the change got out of the site",
              needs=('COMPANY_CORRELATION',)),
    WorldFact('CONTAINMENT_INFRASTRUCTURE', 'facility', 4, False,
              "The doors, the vent splits, the way the power's carved up below the seal - it isn't damage and it isn't neglect. It's a system, built and kept up, to hold one thing shut.",
              lead="what the deep layout actually is",
              needs=('WORKERS_CHOSE_ISOLATION',)),
    WorldFact('WORKERS_MAINTAINING_IT', 'facility', 4, False,
              ...
              needs=('CONTAINMENT_INFRASTRUCTURE', 'CHANGED_HAVE_STRUCTURE')),

    # ---- BAND V  THE SEAL  (chapter 5) --------------------------
    WorldFact('ORE_IS_SOURCE', 'seam', 5, True,
              "Exposure records, the case timing, the ore that went up the shaft - it converges. This seam is where the region's change began. I've been climbing down toward the source the whole way.",
              lead="where the region's outbreak started",
              needs=('EXTRACTION_EXPOSURE', 'CASE_TIMING', 'WORKERS_CHOSE_ISOLATION')),
